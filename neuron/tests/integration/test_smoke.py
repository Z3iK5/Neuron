...
"""Integration smoke test against a LIVE homeserver.

Unlike the unit tests, this talks to a real homeserver. It is automatically
**skipped** unless both ``NEURON_HOMESERVER_URL`` and
``NEURON_HOMESERVER_ADMIN_TOKEN`` are set in the environment and point at a
reachable, admin-capable homeserver (see ``neuron/deploy/compose/README.md``).
...
_BASE_URL = os.environ.get("NEURON_HOMESERVER_URL")
_TOKEN = os.environ.get("NEURON_HOMESERVER_ADMIN_TOKEN")

# Skip the whole module unless we have a homeserver to talk to.
pytestmark = pytest.mark.skipif(
    not (_BASE_URL and _TOKEN),
    reason="Set NEURON_HOMESERVER_URL and NEURON_HOMESERVER_ADMIN_TOKEN to run integration tests.",
)


async def test_server_version_is_reachable() -> None:
    async with AdminClient(_BASE_URL, _TOKEN) as admin:  # type: ignore[arg-type]
