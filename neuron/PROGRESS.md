...
The NEURON "Neural Shield" identity (Concept 1) is defined once in
`neuron_core.branding` — palette, type (Cinzel/Jost), and the mark geometry — and
reused on every surface: the homeserver landing/onboarding pages, the admin
console, the desktop app icon, and the repository assets (`assets/brand/`,
generated by `packaging/make_brand_assets.py`). The root and project READMEs carry
the wordmark and social banner.

---

## QoL — Onboarding (step 1 of 3) — ✅ built

A friendly, in-browser path for people new to Matrix, served by the homeserver
itself (no client needed to get an account):

- **Landing → Get started** (`GET /`): the branded landing page now has a *Get
  started* call to action linking to `/get-started`.
- **Create an account** (`GET`/`POST /get-started`): a small branded form
  (username + password) that runs the existing register flow server-side
  (`AuthService.register(..., inhibit_login=True)`) and shows your new
  `@user:server` Matrix ID on success. Gated on the existing
  `registration_enabled` setting — when registration is closed the form is
  replaced by a note to ask the admin, and `POST` returns `403`. Duplicate /
  invalid usernames re-render the form with the spec error and the typed username
  preserved. All user-controlled values are HTML-escaped.
- **Connect a chat app**: both the get-started and welcome pages spell out the
  homeserver address and the steps to sign in from Element / FluffyChat with the
  Matrix ID + password.

Tests (`tests/neuron_server/test_onboarding.py`): form renders when open / is
hidden when closed; `POST` creates an account and shows the user ID; the created
account can then log in via `/_matrix/client/v3/login`; duplicate username shows
