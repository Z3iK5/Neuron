...
``synapse_base_url`` is read from the ``NEURON_SYNAPSE_BASE_URL`` env var.
...
class NeuronCoreSettings(BaseSettings):
    ...
    inheriting the Synapse-connection and logging settings.
    """
    ...
    # --- Synapse connection -------------------------------------------------
    # Base URL of the Synapse homeserver, e.g. "http://localhost:8008" in dev or
    # "https://matrix.example.org" in production. No trailing slash needed.
    synapse_base_url: str = Field(
        default="http://localhost:8008",
        description="Base URL of the Synapse homeserver to talk to.",
    )

    # A Synapse server-admin access token. REQUIRED for services that use the
    # Admin API. Kept as a SecretStr so it never leaks into logs. In production
    # this is injected via the environment / a mounted secret, never committed.
    synapse_admin_token: SecretStr = Field(
        default=SecretStr(""),
        description="Server-admin access token used for the Synapse Admin API.",
    )

    # How long (seconds) to wait on a single HTTP request to Synapse.
    ...
    def has_admin_token(self) -> bool:
        ...
        return bool(self.synapse_admin_token.get_secret_value())
    ...
    def build_user_id(self, localpart: str) -> str:
        ...
        if localpart.startswith("@"):
            return localpart
        if not self.synapse_server_name:
            raise ValueError(
                "NEURON_SYNAPSE_SERVER_NAME must be set to build a user ID from a localpart."
            )
        return f"@{localpart}:{self.synapse_server_name}"
