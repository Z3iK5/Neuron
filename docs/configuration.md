...
| `NEURON_SERVER_MAX_REMOTE_MEDIA_BYTES` | `104857600` (100 MiB) | Maximum size of a single remote media file fetched over federation and cached locally. Enforced while streaming, before anything is stored. |
...
| `NEURON_SERVER_PUSH_GATEWAY_TIMEOUT_S` | `10` | HTTP timeout for a POST to a mobile push gateway (Sygnal). Delivery is best-effort and runs off the request path, so this only bounds a single gateway call. |
